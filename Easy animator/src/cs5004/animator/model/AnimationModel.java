...
public class AnimationModel implements IAnimator {
  ...
  public List<IShape> getShapeAtTick(int t, int speed) {
    ...
    List<ITransformation> transformationsAtTick = this.getTransList().stream().filter(
        r -> r.getTimePeriod().getStart() * 1000 / speed <= tick && r.getTimePeriod().getEnd()
            * 1000 / speed >= tick).collect(Collectors.toList());
    ...
    for (ITransformation m : transformationsAtTick) {
      ...
      // if a shape have multiple transformations at tick, it may be in the list already.
      // Find the shape and add new transformation to it.
      List<IShape> transShapeList = shapeAtTick.stream().filter(k
          -> k.getName().equals(m.getTransShape().getName())).collect(Collectors.toList());
      IShape transShapeExist = null;
      if (transShapeList.size() != 0) {
        transShapeExist = transShapeList.get(0);
      }

     double ratioOfChange = Math.min(1.0, (tick - m.getTimePeriod().getStart()
          * (1000 / speed)) / ((1000.0 / speed) * (m.getTimePeriod().getEnd()
          - m.getTimePeriod().getStart())));
      ...
      if (m.getTransType() == TransType.NOCHANGE) {
        ...
        // m.getPosition is position before transformation.
        // transShape.getPosition is position after transformation.
        int x = m.getPosition().getX() + (int) ((m.getToPosition().getX() - m.getPosition().getX())
            * ratioOfChange);
        int y = m.getPosition().getY() + (int) ((m.getToPosition().getY() - m.getPosition().getY())
            * ratioOfChange);
        ...
        int shapeProperty2 = m.getShapeProperty().getTwo() + (int) ((m.getToShapeProperty().getTwo()
            - m.getShapeProperty().getTwo()) * ratioOfChange);
        ...
        if (transShapeList.size() == 0) {
          ...
          scaled = true;
          transShapeExist.setShapeProperty(new ShapeProperty(shapeProperty1, shapeProperty2));
        ...
        int r = m.getColor().getRed() + (int) ((m.getToColor().getRed()
            - m.getColor().getRed()) * ratioOfChange);
        int g = m.getColor().getGreen() + (int) ((m.getToColor().getGreen()
            - m.getColor().getGreen()) * ratioOfChange);
        int b = m.getColor().getBlue() + (int) ((m.getToColor().getBlue()
            - m.getColor().getBlue()) * ratioOfChange);
        ...
        if (transShapeList.size() == 0) {
          ...
          transShapeExist.setColor(new Color(r, g, b));
          //transShapeExist already exit in transformationAtTick list, no need to add.
          if (moved && !scaled) {
            transShapeExist.setShapeProperty(m.getShapeProperty());
          } else if (!moved && scaled) {
